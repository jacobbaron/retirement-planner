...
class TestLocalStorageService:
    ...
    def test_store_file_with_metadata(self, local_storage):
        ...
        metadata = {"content_type": "text/plain", "description": "Test file"}
    ...
    def test_store_file_from_path(self, local_storage, temp_dir):
        ...
        # Store it
        stored_path = local_storage.store_file_from_path(
            local_file, "stored/source.txt"
        )
...
@pytest.mark.skipif(
    not S3_AVAILABLE, reason="S3StorageService not available (boto3 not installed)"
)
class TestS3StorageService:
    ...
    def test_retrieve_file(self, s3_storage, mock_s3_client):
        ...
        mock_s3_client.get_object.assert_called_once_with(
            Bucket="test-bucket", Key="test/retrieve.txt"
        )
