...
class TestAssetClass:
    ...
    def test_asset_class_creation(self):
        ...
        asset = AssetClass(
            name="stocks", symbol="^GSPC", source="yahoo", description="S&P 500"
        )
...
class TestHistoricalDataPoint:
    ...
    def test_data_point_creation(self):
        ...
        point = HistoricalDataPoint(
            date=date, value=100.0, asset_class="stocks", source="yahoo"
        )
...
class TestHistoricalDataSet:
    ...
    def test_get_statistics(self):
        ...
        for i in range(12):  # 12 months
            ...
            data_points.append(
                HistoricalDataPoint(
                    date=date, value=value, asset_class="stocks", source="yahoo"
                )
            )
...
class TestCSVSource:
    ...
    def test_csv_source_file_not_found(self):
        ...
        with pytest.raises(StorageError):
            source.fetch_data(
                "nonexistent.csv", datetime(2023, 1, 1), datetime(2023, 2, 1)
            )
...
class TestHistoricalDataManager:
    ...
    def test_acquire_data_unknown_source(self):
        ...
        with pytest.raises(StorageError):
            manager.acquire_data(
                asset, datetime(2023, 1, 1), datetime(2023, 2, 1), "unknown"
            )
    ...
    def test_store_and_load_dataset(self):
        ...
        mock_storage.store_file.return_value = (
            "historical_data/stocks_20230101_20230101.json"
        )
        mock_storage.retrieve_file.return_value = dataset.model_dump_json().encode(
            "utf-8"
        )
...
class TestHistoricalReturnsProvider:
    ...
    def test_generate_returns(self):
        ...
        provider.returns_matrix = np.array(
            [[0.01, 0.02, -0.01, 0.03]]
        )  # 1 asset, 4 periods
