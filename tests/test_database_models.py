...
from app.database.base import create_tables, drop_tables, get_session
from app.database.models import LedgerRow, Run, User, VersionedScenario
...
from app.models.scenario import Scenario as PydanticScenario
from app.models.scenario import Strategy
...
class TestDatabaseModels:
    ...
    def sample_scenario_data(self):
        ...
        return PydanticScenario(
            ...
            strategy=Strategy(withdrawal_method="fixed_real", withdrawal_rate=0.04),
        )
    ...
    def test_scenario_user_relationship(
        self, db_session, sample_user, sample_scenario_data
    ):
        ...
        user_scenarios = (
            db_session.query(VersionedScenario)
            .filter(VersionedScenario.user_id == sample_user.id)
            .all()
        )
    ...
    def test_run_status_constraint(self, db_session, sample_user, sample_scenario_data):
        ...
        # Test invalid status
        run = Run(versioned_scenario_id=scenario.id, status="invalid_status")
        db_session.add(run)
        with pytest.raises(IntegrityError):
            db_session.commit()
    ...
    def test_ledger_row_creation(self, db_session, sample_user, sample_scenario_data):
        ...
        run = Run(versioned_scenario_id=scenario.id, status="completed")
        db_session.add(run)
        db_session.commit()
        db_session.refresh(run)
    ...
    def test_ledger_row_constraints(
        self, db_session, sample_user, sample_scenario_data
    ):
    ...
    def test_cascade_deletes(self, db_session, sample_user, sample_scenario_data):
        ...
        remaining_rows = (
            db_session.query(LedgerRow).filter(LedgerRow.run_id == run.id).all()
        )
        ...
        remaining_runs = (
            db_session.query(Run).filter(Run.versioned_scenario_id == scenario.id).all()
        )
    ...
    def test_indexes_exist(self, db_session):
        ...
        users_by_email = (
            db_session.query(User).filter(User.email == "index@example.com").all()
        )
        assert len(users_by_email) == 1

        scenarios_by_user = (
            db_session.query(VersionedScenario)
            .filter(VersionedScenario.user_id == user.id)
            .all()
        )
    ...
    def test_pydantic_integration(self, db_session, sample_user):
        ...
        pydantic_scenario = PydanticScenario(
            household=Household(primary_age=40, filing_status="single", state="NY"),
            accounts=Accounts(),
            liabilities=Liabilities(),
            incomes=Incomes(),
            expenses=Expenses(),
            ...
            strategy=Strategy(
                withdrawal_method="fixed_percentage", withdrawal_rate=0.035
            ),
        )
