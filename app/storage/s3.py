...
from .base import StorageService, StorageError, StorageNotFoundError, StoragePermissionError
...
class S3StorageService(StorageService):
    ...
    def __init__(
        ...
        """
        if not S3_AVAILABLE:
            raise StorageError("boto3 is not installed. Install with: pip install boto3")
    ...
    def store_file(
        ...
        try:
            ...
            self.s3_client.put_object(
                ...
                Key=s3_key,
                Body=content,
                ContentType=metadata.get('content_type', 'application/octet-stream') if metadata else 'application/octet-stream'
            )
            ...
            error_code = e.response['Error']['Code']
            if error_code == '403':
                raise StoragePermissionError(f"Permission denied storing file {file_path}: {e}")
            else:
    ...
    def retrieve_file(self, file_path: str) -> bytes:
        ...
        try:
            ...
            if error_code == 'NoSuchKey':
                ...
                raise StoragePermissionError(f"Permission denied retrieving file {file_path}: {e}")
            else:
    ...
    def delete_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            error_code = e.response['Error']['Code']
            if error_code == '403':
                raise StoragePermissionError(f"Permission denied deleting file {file_path}: {e}")
            else:
    ...
    def get_file_metadata(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                stored_metadata = json.loads(metadata_response['Body'].read().decode('utf-8'))
            ...
            if error_code == '404':
                ...
                raise StoragePermissionError(f"Permission denied getting metadata for file {file_path}: {e}")
            else:
    ...
    def list_files(self, prefix: str = "") -> List[str]:
        ...
        try:
            ...
            error_code = e.response['Error']['Code']
            if error_code == '403':
                raise StoragePermissionError(f"Permission denied listing files with prefix {prefix}: {e}")
            else:
