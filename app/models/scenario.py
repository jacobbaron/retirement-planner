...
class Child(BaseModel):
    ...
    college_start_year: int = Field(..., ge=1900, le=2100, description="Year college starts")
...
class Household(BaseModel):
    ...
    primary_age: int = Field(..., ge=18, le=100, description="Age of primary household member")
    spouse_age: Optional[int] = Field(None, ge=18, le=100, description="Age of spouse (null if single)")
    filing_status: Literal[
        "single", "married_filing_jointly", "married_filing_separately", 
        "head_of_household", "qualifying_widow"
    ] = Field(..., description="Tax filing status")
    state: str = Field(..., min_length=2, max_length=2, description="State of residence (2-letter code)")
    children: List[Child] = Field(default_factory=list, description="List of children in household")
...
class TaxableAccount(InvestmentAccount):
    ...
    cost_basis: Optional[float] = Field(None, ge=0, description="Cost basis for tax calculations")


class Traditional401kAccount(InvestmentAccount):
    """Traditional 401(k) account."""
    employer_match: Optional[float] = Field(None, ge=0, description="Annual employer match amount")
...
class CashAccounts(BaseModel):
    ...
    money_market: float = Field(default=0, ge=0, description="Money market account balance")


class Accounts(BaseModel):
    """All financial accounts and assets."""
    taxable: List[TaxableAccount] = Field(default_factory=list, description="Taxable investment accounts")
    traditional_401k: List[Traditional401kAccount] = Field(default_factory=list, description="Traditional 401(k) accounts")
    roth_401k: List[Roth401kAccount] = Field(default_factory=list, description="Roth 401(k) accounts")
    traditional_ira: List[TraditionalIRAAccount] = Field(default_factory=list, description="Traditional IRA accounts")
    roth_ira: List[RothIRAAccount] = Field(default_factory=list, description="Roth IRA accounts")
    hsa: List[HSAAccount] = Field(default_factory=list, description="Health Savings Accounts")
    college_529: List[College529Account] = Field(default_factory=list, description="529 college savings accounts")
    cash: CashAccounts = Field(default_factory=CashAccounts, description="Cash and cash equivalents")
...
class Mortgage(BaseModel):
    ...
    property_value: Optional[float] = Field(None, ge=0, description="Current property value")
    extra_payment: float = Field(default=0, ge=0, description="Monthly extra payment amount")
...
class StudentLoan(BaseModel):
    ...
    loan_type: Literal["federal", "private"] = Field(default="federal", description="Type of loan")
...
class Liabilities(BaseModel):
    ...
    mortgages: List[Mortgage] = Field(default_factory=list, description="Mortgage loans")
    student_loans: List[StudentLoan] = Field(default_factory=list, description="Student loan debt")
    credit_cards: List[CreditCard] = Field(default_factory=list, description="Credit card debt")
    auto_loans: List[AutoLoan] = Field(default_factory=list, description="Auto loan debt")


class Salary(BaseModel):
    """Salary income information."""
    person: Literal["primary", "spouse"] = Field(..., description="Which household member")
...
class Incomes(BaseModel):
    ...
    salary: List[Salary] = Field(default_factory=list, description="Salary income by person")
    social_security: List[SocialSecurity] = Field(default_factory=list, description="Social Security benefits")
    pension: List[Pension] = Field(default_factory=list, description="Pension income")
    other: List[OtherIncome] = Field(default_factory=list, description="Other income sources")


class HousingExpenses(BaseModel):
    """Housing-related expenses."""
    mortgage_payment: float = Field(default=0, ge=0, description="Monthly mortgage payment")
...
class TransportationExpenses(BaseModel):
    ...
    auto_payment: float = Field(default=0, ge=0, description="Monthly auto loan payment")
...
class HealthcareExpenses(BaseModel):
    """Healthcare expenses."""
    insurance: float = Field(default=0, ge=0, description="Monthly health insurance premium")
    out_of_pocket: float = Field(default=0, ge=0, description="Annual out-of-pocket healthcare costs")
    medicare: float = Field(default=0, ge=0, description="Monthly Medicare premium (when applicable)")
...
class LumpyExpense(BaseModel):
    ...
    category: Literal["home_improvement", "vehicle", "medical", "education", "other"] = Field(
        default="other", description="Expense category"
    )


class Expenses(BaseModel):
    """All expense categories and projections."""
    housing: HousingExpenses = Field(default_factory=HousingExpenses, description="Housing-related expenses")
    transportation: TransportationExpenses = Field(default_factory=TransportationExpenses, description="Transportation expenses")
    healthcare: HealthcareExpenses = Field(default_factory=HealthcareExpenses, description="Healthcare expenses")
    food: float = Field(default=0, ge=0, description="Monthly food expenses")
    entertainment: float = Field(default=0, ge=0, description="Monthly entertainment expenses")
    travel: float = Field(default=0, ge=0, description="Annual travel expenses")
    education: float = Field(default=0, ge=0, description="Annual education expenses (for children)")
    other: float = Field(default=0, ge=0, description="Monthly other expenses")
    lumpy_expenses: List[LumpyExpense] = Field(default_factory=list, description="One-time or irregular expenses")
...
class LifeInsurance(BaseModel):
    ...
    policy_type: Literal["term", "whole", "universal"] = Field(default="term", description="Type of policy")
...
class DisabilityInsurance(BaseModel):
    ...
    annual_premium: float = Field(..., ge=0, description="Annual premium amount")
    elimination_period: int = Field(default=90, ge=0, description="Days before benefits begin")
...
class Policies(BaseModel):
    ...
    life_insurance: List[LifeInsurance] = Field(default_factory=list, description="Life insurance policies")
    disability_insurance: List[DisabilityInsurance] = Field(default_factory=list, description="Disability insurance policies")
    long_term_care: List[LongTermCareInsurance] = Field(default_factory=list, description="Long-term care insurance")
...
class Correlations(BaseModel):
    ...
    stocks_bonds: float = Field(default=0.2, ge=-1, le=1, description="Stocks-Bonds correlation")
    stocks_cash: float = Field(default=0.0, ge=-1, le=1, description="Stocks-Cash correlation")
    bonds_cash: float = Field(default=0.1, ge=-1, le=1, description="Bonds-Cash correlation")


class MarketModel(BaseModel):
    """Market assumptions and return expectations."""
    expected_returns: ExpectedReturns = Field(..., description="Expected annual returns by asset class")
    volatility: Volatility = Field(..., description="Annual volatility by asset class")
    correlations: Correlations = Field(default_factory=Correlations, description="Correlation matrix between asset classes")
    inflation: float = Field(default=0.025, ge=0, le=1, description="Expected annual inflation rate")
    simulation_type: Literal["deterministic", "monte_carlo", "historical_bootstrap"] = Field(
        default="monte_carlo", description="Type of simulation to run"
    )
    num_simulations: int = Field(default=10000, ge=1, le=100000, description="Number of Monte Carlo simulations")


class Rebalancing(BaseModel):
    """Portfolio rebalancing strategy."""
    frequency: Literal["annual", "semi_annual", "quarterly", "monthly", "threshold_based"] = Field(
        default="annual", description="Rebalancing frequency"
    )
    threshold: float = Field(default=0.05, ge=0, le=1, description="Rebalancing threshold (for threshold_based)")
...
class TaxOptimization(BaseModel):
    ...
    tax_loss_harvesting: bool = Field(default=True, description="Enable tax loss harvesting")
...
class SocialSecurityStrategy(BaseModel):
    ...
    primary_claim_age: int = Field(default=67, ge=62, le=70, description="Age to claim primary Social Security")
    spouse_claim_age: int = Field(default=67, ge=62, le=70, description="Age to claim spousal Social Security")
    file_and_suspend: bool = Field(default=False, description="Use file and suspend strategy")


class Strategy(BaseModel):
    """Withdrawal and investment strategy."""
    withdrawal_method: Literal["fixed_real", "fixed_percentage", "vpw", "guyton_klinger"] = Field(
        default="fixed_real", description="Method for calculating annual withdrawals"
    )
    withdrawal_rate: float = Field(default=0.04, ge=0, le=1, description="Initial withdrawal rate (for fixed methods)")
    withdrawal_start_year: Optional[int] = Field(None, ge=1900, le=2100, description="Year to start withdrawals")
    rebalancing: Rebalancing = Field(default_factory=Rebalancing, description="Portfolio rebalancing strategy")
    tax_optimization: TaxOptimization = Field(default_factory=TaxOptimization, description="Tax optimization strategies")
    social_security_strategy: SocialSecurityStrategy = Field(
        default_factory=SocialSecurityStrategy, description="Social Security claiming strategy"
    )
...
class ScenarioMetadata(BaseModel):
    ...
    created_date: Optional[datetime] = Field(None, description="When scenario was created")
    modified_date: Optional[datetime] = Field(None, description="When scenario was last modified")
...
class Scenario(BaseModel):
    ...
    household: Household = Field(..., description="Household composition and demographics")
    ...
    market_model: MarketModel = Field(..., description="Market assumptions and return expectations")
    strategy: Strategy = Field(..., description="Withdrawal and investment strategy")
    scenario_metadata: Optional[ScenarioMetadata] = Field(None, description="Metadata about the scenario")
