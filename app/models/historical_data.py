...
class HistoricalDataSet(BaseModel):
    ...
    frequency: str = Field(
        default="monthly", description="Data frequency (daily, monthly, annual)"
    )
    ...
    def validate_data_points(
        ...
        for i in range(1, len(v)):
            ...
            if (curr_date - prev_date).days > 60:
                logger.warning(
                    f"Large gap detected between {prev_date} and {curr_date}"
                )
    ...
    def get_statistics(self) -> Dict[str, float]:
        ...
        return {
            ...
            "total_return": float(
                (self.data_points[-1].value / self.data_points[0].value) - 1
            ),
    ...
    def fetch_data(
        ...
        try:
            ...
            for row in csv.DictReader(csv_data.splitlines()):
                ...
                data_points.append(
                    HistoricalDataPoint(
                        date=date, value=close_price, asset_class=symbol, source="yahoo"
                    )
                )
        ...
        try:
            ...
            for observation in data.get("observations", []):
                if observation.get("value") != ".":  # Skip missing values
                    ...
                    data_points.append(
                        HistoricalDataPoint(
                            date=date, value=value, asset_class=series_id, source="fred"
                        )
                    )
...
class HistoricalDataManager:
    ...
    def add_data_source(
        self, name: str, source: Union[YahooFinanceSource, FREDSource, CSVSource]
    ) -> None:
        """Add a data source."""
        ...
        for i in range(1, len(dataset.data_points)):
            ...
            if (curr_date - prev_date).days > 60:
                validation_results["issues"].append(
                    f"Large gap between {prev_date} and {curr_date}"
                )
    ...
    def store_dataset(self, dataset: HistoricalDataSet) -> str:
        ...
        try:
            ...
            logger.info(
                f"Stored dataset for {dataset.asset_class.name} at {storage_path}"
            )
            return storage_path
    ...
    def load_dataset(self, storage_path: str) -> HistoricalDataSet:
        ...
        try:
            ...
            logger.info(
                f"Loaded dataset for {dataset.asset_class.name} from {storage_path}"
            )
            return dataset
            ...
            for file_path in files:
                if file_path.endswith(".json"):
                    try:
                        ...
                        datasets.append({"path": file_path, "metadata": metadata})
                    except Exception as e:
...
def create_default_data_sources() -> (
    Dict[str, Union[YahooFinanceSource, FREDSource, CSVSource]]
):
    ...
    csv_config = DataSourceConfig(
        name="csv", base_url=None, api_key=None, rate_limit=None, timeout=30
    )
