...
class SimpleScenarioVersioningService:
    ...
    def create_version(
        ...
        versioned_scenario = VersionedScenario(
            ...
            scenario_data=scenario_data.model_dump(
                mode="json"
            ),  # Full copy with JSON serialization
    ...
    def list_user_scenarios(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        scenarios = (
            self.db.query(VersionedScenario)
            .filter(VersionedScenario.user_id == user_id)
            .order_by(VersionedScenario.created_at.desc())
            .all()
        )
    ...
    def compare_versions(
        self, scenario_id_1: str, scenario_id_2: str
    ) -> Dict[str, Any]:
        """
        ...
        diff = DeepDiff(
            ...
            ignore_order=True,
            exclude_paths=[
                "root['metadata']['created_at']",
                "root['metadata']['updated_at']",
            ],
        )
    ...
    def get_version_history(self, scenario_id: str) -> List[Dict[str, Any]]:
        ...
        while current:
            ...
            current = (
                self.db.query(VersionedScenario)
                .filter(VersionedScenario.parent_version_id == current.id)
                .first()
            )
    ...
    def _get_next_version(
        self, user_id: int, parent_version_id: Optional[int] = None
    ) -> str:
        ...
        if parent_version_id:
            ...
            parent = (
                self.db.query(VersionedScenario)
                .filter(VersionedScenario.id == parent_version_id)
                .first()
            )
            if parent:
                ...
                existing_branches = (
                    self.db.query(VersionedScenario)
                    .filter(VersionedScenario.parent_version_id == parent_version_id)
                    .count()
                )
